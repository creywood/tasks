...
public abstract class CaldavDao {
  ...
  public abstract CaldavTask getTaskByRemoteId(String remoteId);
  ...
  @Query(
          "SELECT * FROM caldav_tasks "
                  + "WHERE cd_calendar = :calendar "
                  + "AND cd_remote_parent != '' "
                  + "AND cd_parent = 0 ")
  public abstract List<CaldavTask> getTasksNeedingParentUpdate(String calendar);
  ...
  @Query("SELECT caldav_tasks.* FROM caldav_tasks INNER JOIN tasks ON _id = cd_task WHERE cd_parent = :id AND deleted = 0")
  public abstract List<CaldavTask> getImmediateChildren(Long id);
}
